...
import { MigrationState, AudioInterviewResult } from './types';
import { SAMPLE_PROGRAMS, INITIAL_COBOL_EXAMPLE } from './constants';
import { analyzeCobolCode, transformToSpringBoot, generateCloudConfig, analyzeCobolScreenshot, analyzeSystemVideo } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleApiError = (err: any) => {
    ...
    if (isQuotaError || isNotFoundError) {
      setHasApiKey(false);
      return isQuotaError 
        ? "Quota exceeded. Please select a paid API key via the 'Key' button in the header."
        : "Selected API project not found. Please re-select a valid project with billing enabled.";
    }
        ...
        callbacks: {
          ...
          onmessage: async (msg) => {
            ...
            if (msg.serverContent?.inputTranscription) {
              setTranscriptions(prev => [...prev, "User: " + msg.serverContent!.inputTranscription!.text]);
            }
          ...
          onerror: (e: any) => {
            setIsInterviewing(false);
            setState(prev => ({ ...prev, error: handleApiError(e) }));
          }
        },
        config: {
  ...
  const endInterview = async () => {
    ...
    const summary: AudioInterviewResult = {
    ...
    setState(prev => ({ ...prev, audioInterview: summary }));
  };

  const startMigration = async () => {
    try {
      if (!hasApiKey && window.aistudio) {
        if (window.confirm("A paid API key is required for Gemini 3 Pro reasoning. Open key selector?")) {
          await handleOpenKeySelector();
        } else {
          return;
        }
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {(!hasApiKey) && window.aistudio && (
               <button onClick={handleOpenKeySelector} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-xs text-amber-400 animate-pulse font-bold">
            ...
            <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-6 flex flex-col gap-4 text-red-400 mb-8 shadow-xl animate-in fade-in slide-in-from-top-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 font-bold">
                  <AlertTriangle size={20} /> Migration Error
                </div>
                {state.error.includes("Quota") && window.aistudio && (
                  <button onClick={handleOpenKeySelector} className="flex items-center gap-2 px-3 py-1 bg-red-500/20 rounded-lg text-[10px] font-bold uppercase tracking-wider border border-red-500/30 hover:bg-red-500/30 transition-all">
                    Update Key
                  </button>
                )}
              </div>
              ...
              {state.error.includes("Quota") && (
                <p className="text-[11px] text-slate-500 border-t border-red-500/10 pt-4">
                  Note: Deep reasoning and multimodal analysis (Pro) requires a paid API key from a project with billing enabled. 
                  <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noopener noreferrer" className="ml-1 text-red-400/80 hover:underline">View Billing Docs</a>
                </p>
              )}
            </div>
          )}
                      ...
                      <div className="space-y-4 animate-in slide-in-from-top-2">
                        <div className="flex flex-col items-center justify-center bg-slate-900/40 p-10 border-2 border-dashed border-slate-700 rounded-xl gap-4">
                          ...
                          <div className="text-center">
                            <h4 className="font-bold text-slate-200">Interactive Requirement Gathering</h4>
                            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">Gemini 2.5 Live Agent will interview you to extract deep business context missing from source code.</p>
                          </div>
                          {!isInterviewing ? (
                        ...
                        {state.audioInterview && (
                          <div className="bg-slate-900 p-6 rounded-xl border border-blue-500/20 space-y-4 shadow-xl">
                             <div className="flex items-center gap-2 text-blue-400 font-bold uppercase text-xs tracking-widest"><ShieldCheck size={14}/> Interview Artifacts</div>
                             <p className="text-xs text-slate-400 leading-relaxed">{state.audioInterview.summary}</p>
                             <div className="flex flex-wrap gap-2">
                               {state.audioInterview.extractedRequirements.map((r,i)=><span key={i} className="text-[10px] px-3 py-1.5 bg-blue-500/10 text-blue-300 border border-blue-500/20 rounded-full font-medium">{r}</span>)}
                             </div>
                          </div>
                        )}
                      </div>
                ...
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-10 flex flex-col items-center text-center space-y-6 shadow-2xl">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-slate-400 text-sm max-w-lg mx-auto leading-relaxed">System logic has been successfully extracted, validated, and modernized into a cloud-native Spring Boot service.</p>
                    </div>
      ...
      <footer className="border-t border-slate-800 py-12 bg-slate-900/50 mt-20">
        <div className="max-w-7xl mx-auto px-4 text-center space-y-4">
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.2em]">
            Autonomous Modernization Protocol v3.2 | Neural Analysis Engine
          </p>
          <div className="flex justify-center gap-6 text-[10px] font-bold text-slate-600">
            <span className="hover:text-indigo-400 transition-colors cursor-help">Security Policy</span>
            <span className="hover:text-indigo-400 transition-colors cursor-help">System Audit</span>
            <span className="hover:text-indigo-400 transition-colors cursor-help">LLM Transparency</span>
          </div>
        </div>
      </footer>
    </div>
