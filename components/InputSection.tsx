...
const InputSection: React.FC = () => {
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
          ...
          onmessage: async (msg) => {
            if (msg.serverContent?.outputTranscription)
              setTranscriptions((prev) => [
                ...prev,
                'AI: ' + msg.serverContent!.outputTranscription!.text,
              ]);
            if (msg.serverContent?.inputTranscription)
              setTranscriptions((prev) => [
                ...prev,
                'User: ' + msg.serverContent!.inputTranscription!.text,
              ]);
        ...
        config: {
          ...
          systemInstruction:
            'You are an expert analyst modernizing COBOL. Gather requirements from the user.',
        },
      });
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          },
          {
            ...
            label: 'Behavior',
            color: 'purple',
            desc: 'Walkthrough Scan',
          },
          { id: 'audio', Icon: Mic, label: 'Interview', color: 'blue', desc: 'Context Synthesis' },
        ].map((mode) => (
          <button
            key={mode.id}
            onClick={() => setAnalysisMode(mode.id as 'code' | 'image' | 'video' | 'audio')}
          ...
          >
            <mode.Icon
              size={24}
              className={analysisMode === mode.id ? `text-indigo-400` : 'text-slate-500'}
            />
                ...
                <textarea
                  ...
                  placeholder={
                    selectedSampleId === 'custom'
                      ? 'Paste your legacy COBOL source code here for neural analysis...'
                      : ''
                  }
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-4">
            ...
            {!uploadedImage ? (
              ...
              >
                <Upload
                  size={48}
                  className="text-slate-500 group-hover:text-emerald-500 transition-all mb-4"
                />
                <h4 className="font-bold text-lg">Screenshot OCR Workspace</h4>
                <p className="text-xs text-slate-500 mt-2 text-center">
                  Incorporate mainframe frames to auto-discover schema fields
                </p>
              </div>
            ) : (
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-white animate-pulse" /> Synchronize
                    Frame Intelligence
                  </div>
                </button>
              </div>
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-4">
            ...
            {!uploadedVideo ? (
              ...
              >
                <Video
                  size={48}
                  className="text-slate-500 group-hover:text-purple-500 mb-4 transition-all"
                />
                <h4 className="font-bold text-lg text-center">Behavioral Walkthrough Scan</h4>
                <p className="text-xs text-slate-500 mt-2">
                  Walkthrough analysis to detect state transitions
                </p>
              </div>
            ) : (
              <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-mono uppercase">
                        Staged for Behavioral Analysis
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <button
                  onClick={() =>
                    uploadedVideo && handleVideoAnalysis(uploadedVideo.data, uploadedVideo.type)
                  }
            ...
            {state.videoAnalysis && (
              <div className="bg-slate-900 border border-purple-500/20 rounded-2xl p-8 space-y-6 animate-in fade-in shadow-2xl">
                <h4 className="text-[10px] font-bold text-purple-400 uppercase tracking-widest flex items-center gap-2">
                  <Sparkles size={14} /> UI Behavioral Map
                </h4>
                <div className="grid grid-cols-2 gap-6">
                  <div className="bg-slate-800/40 p-5 rounded-xl border border-slate-800">
                    ...
                    <ul className="text-xs list-disc pl-4 mt-4 space-y-2 text-slate-300 font-medium">
                      {state.videoAnalysis.navigationPatterns.map((p, i) => (
                        <li key={i}>{p}</li>
                      ))}
                    </ul>
                  </div>
                  <div className="bg-slate-800/40 p-5 rounded-xl border border-slate-800">
                    ...
                    <ul className="text-xs list-disc pl-4 mt-4 space-y-2 text-slate-300 font-medium">
                      {state.videoAnalysis.uiComponents.map((p, i) => (
                        <li key={i}>{p}</li>
                      ))}
                    </ul>
                  </div>
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4">
            ...
            {audioSubMode === 'live' ? (
              <div className="flex flex-col items-center justify-center p-20 border-2 border-dashed border-slate-800 rounded-3xl gap-6 shadow-inner relative">
                <div
                  className={`p-6 rounded-3xl bg-blue-600/10 text-blue-500 transition-all duration-700 ${isInterviewing ? 'animate-pulse scale-110 shadow-[0_0_50px_rgba(59,130,246,0.2)]' : ''}`}
                >
                  <Mic size={64} />
                </div>
                <div className="text-center space-y-2">
                  ...
                  <p className="text-xs text-slate-500 max-w-sm">
                    Collaborate with the Native Audio Agent to discover functional context.
                  </p>
                </div>
                {!isInterviewing ? (
                  <button
                    onClick={startInterview}
                    className="px-10 py-4 bg-blue-600 rounded-2xl font-bold hover:bg-blue-500 transition-all shadow-xl shadow-blue-500/20 active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className="px-10 py-4 bg-red-600 rounded-2xl font-bold animate-pulse shadow-xl shadow-red-500/20 flex items-center gap-2"
                  >
                ...
                {transcriptions.length > 0 && (
                  <div className="w-full mt-6 bg-slate-950/50 p-6 rounded-2xl border border-blue-500/10 max-h-48 overflow-y-auto space-y-3 custom-scrollbar">
                    {transcriptions.map((t, i) => (
                      <div
                        key={i}
                        className={`text-xs p-3 rounded-lg border shadow-sm ${t.startsWith('AI:') ? 'bg-blue-600/5 text-blue-200 border-blue-500/10' : 'bg-slate-800 text-slate-400 border-slate-700'}`}
                      >
                        {t}
                      </div>
                    ))}
              ...
              <div className="space-y-6">
                <input
                  ...
                  accept="audio/*"
                  onChange={(e) =>
                    e.target.files && setPendingAudioFiles(Array.from(e.target.files))
                  }
                />
                {pendingAudioFiles.length === 0 ? (
                  ...
                  >
                    <FileText
                      size={64}
                      className="text-indigo-400 group-hover:scale-110 transition-transform mb-4"
                    />
                    <div className="text-center">
                      <h4 className="text-xl font-bold">Ingest Recorded Interviews</h4>
                      <p className="text-xs text-slate-500 mt-2 font-medium">
                        Upload stakeholder sessions for batch requirement analysis
                      </p>
                    </div>
                  </div>
                ) : (
                    ...
                    <div className="bg-slate-800/50 rounded-3xl border border-indigo-500/30 p-8 shadow-2xl">
                      <div className="flex items-center justify-between mb-6">
                        <h4 className="text-sm font-bold uppercase tracking-widest text-indigo-400 flex items-center gap-2">
                          <Files size={16} /> Staged Stakeholder Data
                        </h4>
                        <button
                          onClick={() => setPendingAudioFiles([])}
                      ...
                      <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                        {pendingAudioFiles.map((file, idx) => (
                          <div
                            key={idx}
                            className="flex items-center justify-between bg-slate-900/50 p-4 rounded-xl border border-slate-700"
                          >
                            <span className="text-xs font-mono text-slate-300 truncate">
                              {file.name}
                            </span>
                            <span className="text-[10px] text-indigo-400 font-bold uppercase shrink-0 ml-4">
                              Staged
                            </span>
                          </div>
                        ))}
                ...
                {state.audioUploadResults && state.audioUploadResults.length > 0 && (
                  <div className="space-y-6 mt-10">
                    ...
                    {state.audioUploadResults.map((res, i) => (
                      <div
                        key={i}
                        className="bg-slate-800/40 border border-slate-700 rounded-2xl p-6 space-y-4 animate-in fade-in"
                      >
                        <h4 className="text-xs font-bold text-indigo-200 uppercase tracking-widest border-b border-slate-700 pb-2">
                          {res.fileName}
                        </h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-[11px]">
                          <div className="space-y-2">
                            <span className="text-slate-500 font-bold uppercase tracking-tighter block mb-1">
                              Key Requirements
                            </span>
                            <ul className="list-disc pl-4 text-slate-300 space-y-1">
                              {res.requirements.map((r, k) => (
                                <li key={k}>{r}</li>
                              ))}
                            </ul>
                          </div>
  ...
  const renderAnalysis = () => (
    <div className="space-y-10 animate-in fade-in">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-3 py-1 bg-slate-800 rounded-full">
          {selectedSample?.name}
        </span>
      </div>
      ...
      <div className="flex justify-center pt-10">
        <button
          onClick={initializeSynthesis}
          className="px-12 py-5 bg-indigo-600 rounded-2xl font-bold hover:bg-indigo-500 transition-all shadow-2xl flex items-center gap-3 transform hover:-translate-y-1 active:scale-95"
        >
  ...
  const renderTransformation = () => (
    <div className="space-y-10 animate-in fade-in">
      <div className="flex items-center justify-between">
        <h2 className="text-3xl font-bold flex items-center gap-3 tracking-tight">
          <Code2 size={24} className="text-indigo-400" /> Source Tree Synthesis
        </h2>
        <div className="flex items-center gap-2 px-3 py-1 bg-emerald-500/10 text-emerald-400 rounded-full text-[10px] font-bold uppercase tracking-widest border border-emerald-500/20">
          <Sparkles size={12} /> Ready for Review
        </div>
      </div>
  ...
  const renderDeployment = () => (
    <div className="space-y-10 animate-in fade-in">
      <div className="flex items-center justify-between">
        <h2 className="text-3xl font-bold flex items-center gap-3 tracking-tight">
          <Rocket className="text-indigo-400" /> Orchestration Artifacts
        </h2>
      </div>
        ...
        <CodeDisplay
          ...
          files={[
            { path: 'Dockerfile', content: state.deploymentConfig.dockerfile, language: 'docker' },
            {
              ...
              content: state.deploymentConfig.kubernetesYaml,
              language: 'yaml',
            },
            {
              path: 'Helm-Guide.md',
              content: state.deploymentConfig.helmChartDescription,
              language: 'markdown',
            },
          ]}
        />
